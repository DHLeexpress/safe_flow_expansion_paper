...
\begin{itemize}[leftmargin=0pt, itemindent=0pt, label={}]
    ...
    % Finally, we report validity for all 4 task configurations Fig.~\ref{fig:validity-verifier-pedestrian}, average clearance and time-to-goal for challenging OOD tasks~\ref{fig:tradeoff}, and mode coverage results were shown in Fig.~\ref{fig:hero},~\ref{fig:2d_cluttered},~\ref{fig:coverage}(a). 
    To account for varying difficulty across the four tasks, we define the minimum success threshold $P$ as a task-specific hyperparameter, but keep it the same across the $10$ rounds. 
