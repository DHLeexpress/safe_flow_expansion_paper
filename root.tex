...
%%%%%% dropped citations: kurtz2023inverse, aydinoglu2024consensus, Genesis, williams2017model, williams2017info, nguyen2025flowmp, ye2024efficient, mohamed2023gp, makoviychuk2021isaac, dai2025safefm, zeng2021safety

Sampling-based predictive control (SBPC) is an alternative to %gradient-based model 
model predictive control (MPC), where a simple sampling procedure replaces a sophisticated nonlinear optimizer~\cite{posa2014direct}. 
Algorithms in this family include model predictive path integral control (MPPI)~\cite{williams2016aggressive}, predictive sampling~\cite{howell2022predictive}, and cross-entropy methods~\cite{rubinstein1999cross}. SBPC algorithms are particularly well-suited to recent advances in parallel computing architectures~\cite{mjx}, enabling them to scale to complex problems like 
%in-hand 
...
Despite these advantages, SBPCs such as MPPI inherently struggle with safety guarantees. Safety constraints are usually incorporated indirectly through penalty terms in the path planning cost function~\cite{williams2017model, yin2023shield}. While this approach encourages safe behavior, it lacks explicit safety guarantees because the procedure relies on sampling and averaging. Even if individual sampled trajectories satisfy safety constraints, their weighted average may not. 
This limitation motivates our first goal: integrating safety-critical constraints in SBPC.
...
Recent CFM-based works incorporate explicit safety constraints at inference time~\cite{utkarsh2026physics,li2026hardflow}, or use reward-based guidance with subsequent MPC refinement~\cite{mizuta2025unified}. However, generative policies often fail in out-of-distribution (OOD) contexts when demonstrations poorly cover required behaviors~\cite {luo2024potential}, 
and inference-time guidance alone may not sufficiently increase the probability of safe and multi-modal behaviors~\cite{dandapanthula2026we}.
...
We introduce MPPI-DCBF, which  certifies safety in the face of MPPI's averaging process by restricting rollouts to a common convex polytope. 
This approach provides single step safety certification under control-affine dynamics and the entire predicted horizon for LTI systems. 
MPPI-DCBF only requires $25\%$ more computation versus vanilla MPPI. \textbf{(ii)}  
We introduce {\em Safe Flow Expansion} (SFE) for policies trained by flow matching, conditioned on $\gamma$ to tune their safety level. 
...
\textbf{(iii)} Under the stated assumptions, we connect MPPI-DCBF demonstrations to an initial safe set for SFE's expansion analysis
%We formulate our approach in terms of tunable safety parameter $\gamma$. 
and derive a lower bound on the probability that a generated batch contains a valid action at a fixed context and safety parameter $\gamma$.
...
\setlength{\jot}{0pt}
\begin{align}
&\omega_m\!:=\!\exp(-(S^{(m)}\!\!-\!S_{\min})/\lambda),
\;S_{\min}\!:=\!\min_{\ell=1,\ldots,M}S^{(\ell)}.
\label{eq:mppi-weights}\\
&\Delta\mathbf u_{k+i}\!=\!
\frac{\sum_{m=1}^{M}\omega_m\,\varepsilon^{(m)}_{k+i}}
     {\sum_{m=1}^{M}\omega_m},\;
\mathbf u^\star_{k+i}\!\leftarrow\!
\bar{\mathbf u}_{k+i}\!+\!\Delta\mathbf u_{k+i}.
...
% \end{equation}
Only control $\uk^\star$ is applied before replanning. This parallelizable sampling process is well-suited to solving non-convex optimal control problems on GPU hardware. However, a key challenge arises in safety-critical applications. Prior work incorporate DCBF-based penalties toward safety without certifying it~\cite{yin2023shield}, or executes among the lowest-cost rollouts under a CBF filter~\cite{rabiee2024guaranteed}.
...
We train CFM policies on MPPI-DCBF expert action sequences. Let $\hat{\mathbf u}^{\star}$ denote the optimized control sequence returned by~\eqref{eq:mppi-update}, and set
$\z:=\hat{\mathbf u}^{\star}$. The CFM model learns to sample from the distribution of $\z$ conditioned on the observation $\o$ and $\gamma$ (\textit{i.e.,} the safety parameter). For $\epsilon\!\sim\!\mathcal N(0,I_{n_z})$, where $n_z=Hn_u$, and $\z\!\sim\! p_{\mathrm{data}}$, we use an optimal transport conditional probability path $\x_t\!=\!(1-t)\epsilon+t\z$, whose velocity is $\dot{\x}_t=\z-\epsilon$~\cite{lipman2022flow}. 
...
Reward-based fine-tuning improves generated samples while remaining close to the pretrained model for diffusion~\cite{fan2023dpok} and flow models~\cite{domingo2025adjoint}.
...
Active Flow Expansion instead enlarges the \textit{$\tau$-generable set} $\Omega^\tau_\theta\!:=\!\{\x:p_1^\theta(\x)\ge\tau\}$, the region an FM model samples with non-negligible probability under a finite budget~\cite{de2026active}.
...
at every step, then \(h(\x_k)\!\ge\!(1-\gamma)^{k}\,h(\x_0)\), so the fixed set $\mathcal{C}$ is forward invariant~\cite{ames2014cbf,agrawal2017discrete}. Let $\q=\piq(\x)$, with $\piq:\mathbb R^{n_x}\to\mathbb R^d$ linear projection. For each observed convex obstacle $j\in\mathcal O(\xk)$, let $\p_j$ be its closest boundary point to $\qk$ and $\n_j$ its outward unit normal. Assuming $\n_j^\top(\qk-\p_j)>0$, define
\begin{equation}
...
We instead use a common convex inner approximation to guarantee the safety of the MPPI control after weighted averaging, as established below.
...
The policy is pretrained on 200k MPPI-DCBF action sequences from random starts toward a fixed goal among circular obstacles. Evaluation introduces a large central obstacle that blocks the demonstrated short routes in a $5\,\mathrm{m}\times5\,\mathrm{m}$ workspace.
...
Following the social navigation setup of~\cite{mizuta2025unified}, pretraining uses MPPI-DCBF demonstrations among 20 pedestrians moving at $0.5$--$1.0\,\mathrm{m/s}$. Evaluation uses 40 pedestrians at $1.0$--$2.0\,\mathrm{m/s}$.
...
The reward combines a CBF-violation penalty and a goal-reaching reward, weighted by $w_\mathrm{s}$ and $w_\mathrm{g}$~\cite[Eq. 6]{mizuta2025unified}.
...
\vskip -0.3 true in
\end{figure}
CFM-MPPI generates $J=32$ guided priors at each step, refines the eight lowest-cost priors using $M=32$ Gaussian perturbations each, and executes the first control of the lowest-cost refined sequence. It warm-starts the next replan at flow time $t=0.75$. For each task, we selected the weight pair with the best trade-off between collision rate and time-to-goal.
...
More importantly, we demonstrated that SFE not only learns the collision-free path but also discovers completely new safe behaviors in OOD (Fig.~\ref{fig:coverage}). In the 3D hardware experiments, MPPI-DCBF demonstrations collected among vertical cylinders are dominated by planar lateral motion. In the unseen cluttered environment with spheres, the pretrained CFM consequently stays close to the demonstration altitude and collapses into two dominant modes. In contrast, SFE covers all eight lateral modes by \(\theta_{10}\) by shifting a probability mass toward fundamentally new safe modes trained offline.
