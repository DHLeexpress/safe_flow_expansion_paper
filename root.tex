...
These algorithms share deep connections with generative control techniques like diffusion-based~\cite{chi2023diffusion} and flow matching-based policies~\cite{mizuta2025unified}. SBPC demonstrations can \emph{agnostically} deal with a robot's fast nonlinear dynamics and morphology that generative policies learn to imitate~\cite{Kurtz_GPC}, with both data generation and policy training benefiting from parallel computation. 
...
Generative models, such as diffusion and flow matching~\cite{lipman2022flow}, can synthesize motion trajectories and action sequences from diverse, high-dimensional demonstrations~\cite{chi2023diffusion, Kurtz_GPC}, with inference-time guidance to tightly trade off safety and performance~\cite{mizuta2025unified}.
...
This parallelizable sampling process is well-suited to solving non-convex optimal control problems on GPU hardware. However, a key challenge arises in safety-critical applications. Prior work incorporate DCBF-based penalties 
% TODO Not a clear statement
toward safety without certifying it~\cite{yin2023shield}, or executes among the lowest-cost rollouts under a CBF filter~\cite{rabiee2024guaranteed}.
