...
First, MPPI-DCBF rejects rollouts that violate a common convex safety condition before MPPI's weighted averaging, preserving one-step safety. 
...
Second, Safe Flow Expansion (SFE) self-generates actions from a conditional flow matching (CFM) policy, verifies them under the same safety specification, and updates the policy toward valid actions (\textit{i.e.}, those satisfying the safety specifications).
