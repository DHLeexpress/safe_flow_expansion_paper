...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\title{\LARGE \bf
% Finite-horizon Safety for \\Sampling-based Predictive Control and Generative Policy Expansion
...
% valid -> safe
Sampling-based predictive control and generative policies plan robot motion by sampling candidate action sequences. However, sampling alone does not guarantee safety over the prediction horizon, and pretrained policies may poorly cover safe behaviors in out-of-distribution (OOD) environments. 
...
We present a unified framework that incorporates finite-horizon discrete-time control barrier function (DCBF) conditions into both online planning and generative policy learning. 
...
First, MPPI-DCBF rejects rollouts that violate a common convex safety condition before MPPI's weighted averaging, preserving one-step safety. We pretrain a conditional flow matching (CFM) policy on MPPI-DCBF demonstrations.
% Second, for generative policy learning, we introduce Safe Flow Expansion (SFE) to improve safety in OOD domains.  SFE self-generates actions from a conditional flow matching (CFM) policy, verifies them with the same condition, and updates the policy toward its valid actions.
Second, Safe Flow Expansion (SFE) self-generates actions from a pretrained CFM policy, verifies them under the same safety specification, and updates the policy toward safe and valid actions.
% We derive a lower bound on the probability that a batch of generated actions contains a valid action. In 2D and 3D simulation and on Crazyflie hardware, the expanded policy achieves higher validity than the pretrained policy and higher mode coverage than the pretrained and guided baselines, and it replans with far lower latency than online planners.
Under an idealized expansion process, we derive a lower bound on the probability that a generated batch contains a valid action.
In four OOD tasks, expansion improves validity over the pretrained policy. Hardware experiments demonstrate higher observed mode coverage than the pretrained and guided baselines. Single-action generation is $\mathrm{12\times}$ and $\mathrm{50\times}$ faster than MPPI-DCBF and CFM-MPPI replanning, respectively.
\end{abstract}
...
Despite these advantages, SBPCs such as model predictive path integral control (MPPI), struggle with safety guarantees due to their stochastic sampling and averaging procedure. In particular, MPPI safety constraints are usually incorporated indirectly through penalty terms in the path planning cost function~\cite{williams2017model, yin2023shield}. Even if individual sampled trajectories satisfy safety constraints, their weighted average may not. 
% Inherently struggle with 
% This limitation motivates our goal of integrating safety-critical constraints in SBPC.
...
\textbf{Contributions.} SBPC and CFM policies share a sampling-based structure, which can be used to train CFM policies on demonstrations from SPBC \cite{Kurtz_GPC}. In this paper, we show how this structure can incorporate explicit safety conditions for both online MPPI and pretrained CFM policies in motion planning. This new analysis allows us to adapt active flow expansion~\cite{de2026active} to address distribution shift, improving the CFM policy's coverage of valid actions while also maintaining safety. Because a single parameter $\gamma$ modulates our safety constraint and conditions the CFM policy, our framework has the extra benefit of \textit{tunable safety} at inference time.

Our key contributions are as follows: \textbf{(i)} 
We introduce \emph{MPPI-DCBF} (Sec.~\ref{sec:mppi-dcbf}), which certifies safety against MPPI's averaging process by restricting rollouts to a common convex polytope, with modest computational overhead compared to vanilla MPPI.
...
Establishing recursive feasibility remains future work, as with MPC-DCBF~\cite{zeng2021enhancing}.
% MPPI-DCBF requires only $25\%$ more computation than vanilla MPPI. 
\textbf{(ii)}  
We leverage these insights to introduce {\em Safe Flow Expansion} (SFE, Sec.~\ref{sec:sfe}) for CFM policies.
...
\end{equation}
where $f,g$ are continuous, and $\x_k\in\mathcal X\subseteq\mathbb R^{n_x}$ and $\u_k\in\mathcal U\subseteq\mathbb R^{n_u}$ denote the state and control input at step $k$, respectively. At each sampling time $t_k$, the controller plans inputs $\{\u_{k+i}\}_{i=0}^{H-1}$ and predicts states $\{\x_{k+i}\}_{i=1}^{H}$, with $t_{k+i}\!=\!t_k\!+\!i\Delta t$ for a sampling interval $\Delta t$, and horizon $H$.

% \vspace{-2.0mm}
\subsection{Model Predictive Path Integral control}
For control system~\eqref{eq:dynamical_system}, MPPI optimizes a mean control sequence in a convex admissible set $\mathcal U$ over horizon $H$ by sampling and cost-based averaging~\cite{williams2016aggressive}.
It samples $M$ candidate sequences $\mathbf u^{(m)}_{k+i}=\bar{\mathbf u}_{k+i}+\varepsilon^{(m)}_{k+i}$ around the previous solution shifted by one time step, $\bar{\mathbf u}_{k+i}$, to warm-start the optimization, where $\varepsilon^{(m)}_{k+i}\sim\mathcal N(0,\Sigma)$, for $m\!=\!1,\!\ldots,\!M$.
...
\mathbf u^\star_{k+i}
=
\sum_{m=1}^{M}\omega_m\mathbf u^{(m)}_{k+i}\Big/
\sum_{m=1}^{M}\omega_m,
\quad i=0,\ldots,H-1.
...
Prior work attempts to incorporates safety through terms added to the cost function~\cite{williams2016aggressive}, with gradient-based optimization~\cite{yin2023shield}, or by executing the lowest-cost candidate sequences under a CBF filter~\cite{rabiee2024guaranteed}. 
% TODO introduce averaging issue here? or later?
These practical methods do not guarantee safety after convex averaging process~\eqref{eq:mppi-update}.
...
at every step, then \(h(\x_k)\!\ge\!(1-\gamma)^{k}\,h(\x_0)\), so the fixed set $\mathcal{C}$ is forward invariant~\cite{ames2014cbf,agrawal2017discrete}, with smaller $\gamma$ imposing stricter safety margins and restricting the feasible action set~\cite{zeng2021enhancing}.

Our safety specification is obstacle avoidance. We model the robot as a point $\q\!=\!\piq(\x)$,
where $\piq:\!\mathbb R^{n_x}\!\to\!\mathbb R^d$ is a linear projection that maps the robot state to position. 
Let $\mathcal I(\xk)\subset\mathbb N$ index the convex obstacles observed at $\xk$. We represent circular or spherical obstacles as 
\begin{equation}
\mathcal O_j:=\{\q\in\mathbb R^d\!:\!\|\q-\oj\|_2\!\le\! r_j\},
\label{eq:obstacle}
\end{equation}
For each $j\in\mathcal I(\xk)$, let $\p_j$ be the closest point on the obstacle to $\qk$ and $\n_j$ its outward unit normal. The robot is safe with respect to $\mathcal O_j$ when \(\n_j^\top(\qk-\p_j)>0\).
Define
\begin{equation}
...
We train CFM policies on MPPI-DCBF expert action sequences (hereafter, \emph{actions}), so the policy is biased toward safe behaviors. 
%TODO safe?valid?
Let $\hat{\mathbf u}^{\star}$ denote the optimized control sequence~\eqref{eq:mppi-update}, and set
$\z:=\hat{\mathbf u}^{\star}$. 
...
Active Flow Expansion~\cite{de2026active} is a recent active learning framework for generative models. Under proper conditions, it enlarges the \textit{$\tau$-generable set} \(\Omega^\tau_\theta\!:=\!\{\x:p_1^\theta(\x)\ge\tau\}\) of the pretrained model $\theta$ to cover a larger portion of a valid region of the model's output space.
...
Let $J(\hat{\mathbf u};\x_k)$ denote the standard cost in~\eqref{eq:mppicost} of the nominal rollout by the control $\hat{\mathbf u}$ from state $\x_k$.
We aim to guarantee a multi-step safety certificate for online rollout sampling in MPPI and action generation by a CFM policy. 
To this end, we use a safety function $h(\cdot;\x_k)$ constructed on $\x_k$, the observed obstacle $\mathcal I(\xk)$, and fixed over the prediction horizon $i=1,\cdots,H$. 
Its nonnegative superlevel set lies in free space, and $h(\x_k;\x_k)=1$ for notational simplicity.
We formulate safe planning for both MPPI and SFE as
\begin{subequations}
\setlength{\jot}{1pt}
\label{eq:problemdef}
\begin{align}
...
where the bounds $(1-\gamma)^i$ result from the finite-horizon DCBF condition~\eqref{eq:dtcbf}. 
In practice, online planning must construct $h$ in real time, whereas offline policy expansion can afford additional computation to admit more diverse valid actions.
To this end, we propose an efficient geometric formulation of $h$ for MPPI safety, and an optimization-based method that searches for an $h$ to verify diverse $\hat \u$ during offline CFM policy expansion.
...
% Instead, the safety condition in~\eqref{eq:poly} allows us to define a convex inner approximation of the free space. 
% In particular, MPPI uses a common $h$ for all sampled rollouts, while SFE constructs one for each queried action $\hat \u$.
...
\section{Method}
% We propose two methods for constructing $h$, suited to online MPPI planning and offline CFM generation. In particular, we define \emph{nominal} polytope that can be efficiently constructed via half-spaces on the observed obstacles, and \emph{verifier} polytopes as an optimization problem to fit 
% for online MPPI and offline CFM policy expansion.
% In both cases, $h$ is constructed 
\subsection{MPPI-DCBF}
\label{sec:mppi-dcbf}
We propose MPPI with DCBF-based sample rejection (MPPI-DCBF), which accepts only rollouts satisfying the safety specification~\eqref{eq:poly} with $h=h^\text{poly}$ in ~\eqref{eq:poly_single} and uses their control sequences in the weighted update~\eqref{eq:mppi-update}. The following results require at least one accepted rollout, whose inputs lie in $\mathcal U$. Let $\hat\omega_m\ge0$ denote the normalized MPPI weights over accepted rollouts, with $\sum_m\hat\omega_m=1$.
\begin{proposition}
\label{prop:1}
...
$\x^\star_{k+1}\!\in\!\mathcal C_1^{\mathrm{poly}}$, and is thus collision free.
\end{proposition}
...
Thus, the common convex safe sets preserve safety under MPPI averaging for the executed step of~\eqref{eq:dynamical_system} and the full predicted horizon of~\eqref{eq:lti}. These guarantees assume the existence of accepted controls that satisfy the DCBF constraint~\eqref{eq:poly} with $h=h^\text{poly}$ but do not establish recursive feasibility. A similar distinction for MPC-CBF likewise does not ensure recursive feasibility: see~\cite[Remark~1]{zeng2021enhancing}.

\noindent\textbf{Extension: Multi-Body Points and Nonconvex Obstacles.} The safety function in~\eqref{eq:poly_single} applies to general convex obstacles, as in prior work~\cite{agrawal2017discrete, mizuta2025unified}.
MPPI-DCBF can be extended to nonconvex obstacles by representing them as a union of convex sets~\cite{zhang2020optimization,mjx}.
To account for finite robot geometry, we can construct the safety function at multiple body points~\cite{wilkinson2026full}. Let $\pi_c$ map the state to a body point \(\q^{(c)}\!=\!\pi_c(\x)\), with body Jacobian \(J_c\!=\!\partial \pi_c/\partial \x \!\in\! \mathbb R^{3\times n_x}\). For each of the $N_c$ sampled body points, the linearization of $\pi_c$ at $\xk$ is
\begin{equation}
\tilde{\pi}_c(\x;\xk)
:=\pi_c(\xk)+J_c(\xk)(\x-\xk).
\end{equation}
...
Fig.~\ref{fig:robotarm} shows MPPI-DCBF on a Franka FR3 arm, a 7-DOF arm, avoiding nonconvex obstacles in \emph{LIBERO}~\cite{liu2023libero}.
...
% We now turn to expand the generative policy beyond the MPPI-DCBF demonstrations. 
Because half-spaces are a conservative approximation, there may exist problem solutions not found by MPPI-DCBF. In other words, in nonconvex free space, the nominal polytope~\eqref{eq:nominalpolytope} may exclude collision-free rollouts in other modes~\cite{deits2015computing}. We therefore introduce a verification process used in SFE that admits safe actions less conservatively by searching for separating half-spaces around each queried action that satisfy the same safety condition~\eqref{eq:poly}.

For a queried action $\hat{\mathbf u}$, let $\q_{k+i},i\!=\!0,\!\ldots\!,H$, denote the positions of the predicted states $\x_{k+i}$. 
For each circular or spherical obstacle $j\in\mathcal I(\xk)$ in~\eqref{eq:obstacle}, the verifier constructs a separating half-space \(\mathbf a_j^\top(\mathbf q-\qk)\le m_j,\) 
by solving
\noindent
\begin{align}
...
\end{align}
where $\aj$ and $m_j$ are the normal vector and offset relative to the current robot position $\qk$ of the half-space, respectively. Multi-step safety requires only feasibility, and maximizing $m_j$ means choosing a certificate with the largest margin from the current position $\qk$. 
The first constraint admits collision-free queried action by imposing $h$ that satisfies~\eqref{eq:poly}:
\begin{equation}
\label{eq:constructed-safety-function}
h(\xki;\xk)=1-\aj^\top(\qki-\qk)/m_j.
\end{equation}
% which equals to 1 at the current position $\qk$ and is required to remain at least $(1-\gamma)^i$. 
The second constraint in~\eqref{eq:socp} follows from \(
\inf_{\q\in\mathcal O_j}\aj^\top\!(\q\!-\!\qk)\!=\!\aj^\top(\oj\!-\!\qk)\!-\!r_j\|\aj\|_2\!\ge\! m_j.
\)
The remaining constraints bound the scale of the normal vector ($\aj$) and keep the current position strictly inside ($m_{\min}\!>\!0$) the separating half-space.
%The remaining constraints fix the scale of the normal vector of the half-space and enforce the strictly positive margin $m_{\min}\!>\!0$ required by safety. 
Problem~\eqref{eq:socp} is a second-order cone program (SOCP), solved with Clarabel through CVXPY~\cite{goulart2026clarabel}. 
Let $(\aj^\star,m_j^\star)$ be a solution of ~\eqref{eq:socp} for obstacle $j$. The verifier polytope (Fig.~\ref{fig:illustration}) for a queried action is defined by
\begin{equation}
...
\end{equation}
Here, the context $\o$ is the vector that appends the relative goal position, velocity, and the local obstacle observations within the robot's sensing range.
% $j\in\mathcal I(\xk)$ within the robot's sensing range (\textit{e.g.,} Eq.~\eqref{eq:poly_multi}).
...
Now we describe the main objective of SFE: the \textit{validity} of a queried action.
...
%Let $\p_j$ and the outward unit normal $\n_j$ define the nominal face, and let \(d_j:=\n_j^\top(\qk-\p_j)=\|\qk-\oj\|_2-r_j>0.\)
Let $\p_j$ be the point on $\mathcal O_j$ closest to $\qk$, and let $\n_j$ be the outward unit normal at $\p_j$, as used in~\eqref{eq:nominalpolytope}. Define $d_j\!:=\!\n_j^\top(\qk\!-\!\p_j)\!=\!\|\qk\!-\!\oj\|_2\!-\!r_j\!>\!0$.
...
This result shows that the averaged MPPI-DCBF action is a valid action under the conditions of Proposition~\ref{prop:nominal-verifier}. 
For general control-affine dynamics~\eqref{eq:dynamical_system}, the averaged action requires a multi-step verification. 
On the other hand, the verification imposes stricter conditions on the same collision-free rollout as $\gamma$ decreases.
The following proposition formalizes this as an inclusion relation between the valid action sets:
\begin{proposition}
...
We now seek to generate valid actions at the desired safety level $\gamma$, starting by pretraining the CFM policy on a finite set of valid MPPI-DCBF demonstrations.
...
We introduce Safe Flow Expansion (Alg.~\ref{alg:sfe}), which assumes access to a physics simulator that can generate contexts outside the pretraining distribution by varying environment parameters.
In our paper, we vary obstacle geometries and placements and simulate reactive pedestrian dynamics using the social force model~\cite{helbing1995social}.
Within these contexts, SFE iteratively samples actions with uncertainty guidance and updates the pretrained CFM policy toward valid actions beyond the demonstrations, including those excluded by MPPI-DCBF's nominal polytope.
%With the pretrained CFM policy, we introduce Safe Flow Expansion (Alg.~\ref{alg:sfe}) to iteratively sample actions with uncertainty guidance and update the CFM policy toward safe and valid actions beyond the demonstrations, including those excluded by MPPI-DCBF's nominal polytope. 
The policy \(\theta_n\) at round $n$ in Alg.~\ref{alg:sfe} induces the \textit{conditional generable set} associated with the  density lower bound \(\tau\) defined as:
\begin{equation}
...
At each iteration, Alg.~\ref{alg:sfe} samples toward actions with high uncertainty while adhering to the current policy ${\theta_n}$
\begin{equation}
\hat{\mathbf u}
\sim
p_1^{\theta_n}(\hat{\mathbf u}\mid\o,\gamma)\exp\!\left(\sigma_n\!\left(\phi_s(\hat{\mathbf u};\o,\gamma)\right)\!\big/\!\beta\right)
\!\big/\!Z_n(\o,\gamma)
,
\label{eq:importancesampling}
\end{equation}
This sampling distribution is the exact Gibbs solution to
\begin{equation}
\max_q \{
\mathbb{E}_q[\sigma_n(\phi_s(\hat{\mathbf u};\o,\gamma)))]
-\beta\operatorname{KL}(q\parallel p_1^{\theta_n})
\}.
\end{equation}
where $\operatorname{KL}$ denotes the Kullback--Leibler divergence, $\beta>0$ is the parameter that controls the trade-off between exploration and the prior,  
and $Z_n(\o,\gamma)$ is the normalizing constant.
...
%~\footnote{The distribution in~\eqref{eq:importancesampling} is the exact Gibbs solution of \(\max_q \{\mathbb{E}_q[\sigma_n(\phi_s(\hat\u))]\!-\! \beta \operatorname{KL}(q \!\parallel\! p_1^{\theta_n})\},\) where $\operatorname{KL}$ denotes the Kullback-Leibler divergence.}
$\phi_s$ denotes the penultimate layer feature of the CFM vector field $\mathbf v_t^{\theta_n}(\hat \u'\!\mid\!\o,\gamma)$ evaluated at the fixed flow time $t=s$ with noised queried action $\hat \u'=s\hat\u+(1-s)\epsilon$, where $\epsilon$ is Gaussian noise. We modeled $\sigma_n$ as the posterior standard deviation of a Gaussian process fitted on the features $\phi_s$ to the binary observations in the buffer (line 3). For $\phi=\phi_s(\hat{\mathbf u};\o,\gamma)$, define $[\mathbf K_n]_{ij}=k(\phi_i,\phi_j)$ and $[\mathbf k_n(\phi)]_i=k(\phi_i,\phi)$. The posterior standard deviation is
\begin{equation}
...
The verifier $v_{\gamma_k}$ in Def.~\ref{def:verifier} classifies each queried action $\hat{\mathbf u}^{(j)}$ as valid ($j\in\mathcal J_k^+$) or invalid. All queried actions and their labels are then added to $\mathcal D_n$.
...
We repeat acquisition until we collect $P$ safe trajectories, then update the uncertainty buffer with both valid and invalid queried actions. If no queried action is valid, the offline rollout restarts.
\textsc{UpdateCFM} uses hierarchical sampling among valid queried actions $\mathcal D_n^+ \subset \mathcal D_n$, uniformly over $\gamma$ and safe trajectories, and regresses the CFM loss~\eqref{eq:cfm-loss} on these uncertainty-guided action data. 
...
\emph{Proof.} 
By definition of the reachability operator, it holds that
...
For fixed $(\o,\gamma,S_0,\epsilon,\tau,\delta)$, the lower bound in~\eqref{eq:main1} is nondecreasing in both reachability depth $r$ and batch size $J$. Corollary~\ref{thm:safe-action-availability} provides a valid action sampling guarantee for an idealized expansion process. Despite this idealization, we expect the expanded policy $p_1^{\theta_{T^\star}}(\cdot\mid\o,\gamma)$ to have more valid actions to sample from than the pretrained policy.
% TODO explain the results earlier? or later., while improvements, such as validity improvement in rounds (Fig.~\ref{fig:validity}) and the higher clearance and shorter time-to-goal with larger $J$ (Fig.~\ref{fig:tradeoff}), under the implemented Alg.~\ref{alg:sfe} are established empirically. 
...
% We evaluate SFE in 4 OOD tasks: two 2D simulations, a 3D quadrotor simulation, and a hardware flight experiment.

\begin{figure}[!t]
\centering
    \includegraphics[width=1.0\linewidth]{images/pedestrian_v1_traj-comp.pdf}
    \vspace{-7.0mm}
    \caption{In a challenging dense and fast crowd environment, SFE successfully executes valid actions and reach the goal, where $\gamma$ directly controls the valid actions, from conservative ($\gamma\!=\!0.1$) to aggressive behavior ($\gamma\!=\!1.0$).}
...
\noindent\textbf{Evaluation protocol.}
We evaluate the proposed framework in four OOD navigation environments using robots with double-integrator dynamics with $\Delta t\!=\!0.1\mathrm{s}$. We pretrain the policy on in-distribution MPPI-DCBF demonstrations, then conduct offline SFE in OOD, and perform OOD evaluation.
Each expansion round collects $P$ task-specific safe trajectories. Across 3 seeds, we evaluate each policy $\theta_n$ using a 16-step ODE solver on 100 rollouts per task and $\gamma$.
...
In this challenging OOD environment, the policy conditions on the 10-step history of local observations $h^\text{poly}$.
\emph{3D hardware environments}: 
Both both 3D environments, we pretrain among $4-8$ randomly positioned cylinders in a $4\,\mathrm{m}\times4\,\mathrm{m}\times1.5\,\mathrm{m}$ workspace. Evaluation uses one spherical obstacle at the start-goal midpoint (Fig.~\ref{fig:hero}) or 6 randomly positioned spherical obstacles (Fig.~\ref{fig:coverage}). On the Crazyflie, Vicon provides quadrotor and obstacle poses at $50$ Hz. The policy replans at $10$ Hz, and acceleration commands are streamed at $100$ Hz as feedforward terms in 6-DOF setpoints.
...
The CFM velocity network is one MLP with 3 hidden layers, conditioned on $(\o,\gamma)$. We represent local obstacle observations by discretizing the point-robot safety function $h=h^\mathrm{poly}$~\eqref{eq:poly_single} over a $2\mathrm{m}$ sensing range.
% TODO does this make sense? (or its 10-step history in the dynamic-pedestrian task)
A convolutional neural network (CNN) downsamples the angular and radial axes of this discretized observation grid and compresses it into a 64-dim observation token. 

At each SFE step, the CFM policy self-generates $K=32$ proposals and resamples $B=16$ of them for verification under~\eqref{eq:importancesampling}, doubled to \((64,32)\) in the dynamic-pedestrian task. Here $K$ controls exploration and $B$ bounds the verification cost. We use the standard MPC cost~\eqref{eq:mppicost} to select among valid candidates. Uncertainty is the posterior of an RBF Gaussian process (GP) on the $L_2$-normalized 32-dim $\phi_s$ at flow time $s=0.9$ with a task-specific $\beta$ and length scale calibrated from pretrained embeddings. Also, we use constant-velocity pedestrian predictions in the execution cost to rank valid actions~\cite{mizuta2025unified}, while the verifier certifies actions against the observed geometry held fixed over each horizon.

At deployment, SFE generates one action sequence and executes its first control. Practically, we can exploit parallel CFM generation for \emph{cost-based selection} by generating $J>1$ action sequences and executing the first control of the sequence with the lowest rollout cost~\eqref{eq:mppicost}. We evaluate the effect of $J$ on performance and safety in~\Cref{sec:tradeoff}. 

\noindent \textbf{Baseline method} We compare against mode-selective CFM-MPPI~\cite{mizuta2025unified}, which combines reward-guided flow generation with local MPPI refinement.  Given a base pre-trained vector field $\mathbf{v}_t^{\theta_{\text{0}}}(\cdot\!\mid\! \mathbf{o},\gamma)$, the guided velocity field is the local gradient ascent of the combined reward
\begin{equation}
...
We measure validity as the fraction of executed actions for which \eqref{eq:socp} is feasible for every sensed obstacle, over ten expansion rounds in four OOD environments. Fig.~\ref{fig:validity} shows that expansion increases the average validity relative to the pretrained policy in all four environments. 
% This quantitative result supports that Alg.~\ref{alg:sfe} expands the generable set toward valid actions.
This empirical trend across rounds is consistent with the intuition behind our analysis result in \Cref{thm:safe-action-availability}. Repeated CFM updates on valid actions can increase the probability of generating valid actions. 
...
Also, the higher validity at larger $\gamma$ across environments is consistent with~\Cref{prop:gamma-effect}, as the relaxed verifier condition enlarges the set of actions certified as valid.
...
% The lower collision rate complements the validity results by showing improved closed-loop safety in this task.
We hypothesize that higher action validity improves closed-loop safety in this task because the multi-step safety certificate considers future states to mitigate myopic control.
%We hypothesize that the increased action validity improves closed-loop safety in this task, because the multi-step safety certificate includes pointwise safety for the dynamics we chose. 
...
\begin{figure*}[t]
    ...
    \vspace{-7.0mm}
    \caption{\textbf{Safe Flow Expansion increases mode coverage in an OOD obstacle geometry and placement at a fraction of online planners' latency.} 
    Each rollout is assigned a lateral mode based on its left/right choice at each gate, with \(2^3=8\) possible modes. Mode coverage counts the distinct modes observed over 40 random rollouts.
    %, and we show one representative rollout for each observed mode. 
    \textbf{(a)} MPPI-DCBF demonstrations among vertical cylinders in simulation, Crazyflie hardware rollouts in one fixed unseen sphere layout for the pretrained CFM ($\theta_0$), the policy after five expansion rounds ($\theta_5$), the SFE policy ($\theta_{10}$), and CFM-MPPI ($\gamma=0.3$ throughout). 
    \textbf{(b)} Mean replanning time per step on an NVIDIA RTX 4060 Ti GPU, using 512 rollouts for each MPPI variant and 32 guided proposals for CFM-MPPI.
    }
    \vskip -0.25 true in
    \label{fig:coverage}
\end{figure*}
...
Fig.~\ref{fig:2d_cluttered}, Fig.~\ref{fig:pedestrian}, and Fig.~\ref{fig:coverage} show that SFE learns collision-free paths in different OOD domains: unseen large obstacle geometry, dense and faster-moving crowd, and unseen obstacle geometry and placements. 
% SFE discovers completely new safe behaviors in OOD beyond those represented in its pretraining demonstrations by 
...
In the 3D hardware experiments, the MPPI-DCBF demonstrations collected among vertical cylinders. These initial pretraining data are dominated by planar lateral motion, as the planner assigns low cost under~\eqref{eq:mppicost} to those rollouts. In OOD with sphere obstacles, the pretrained CFM consequently stays close to the demonstration altitude and collapses into two dominant modes. In contrast, SFE covers all eight lateral modes by \(\theta_{10}\) by shifting a probability mass toward fundamentally new safe modes trained offline.

SFE enables this safe mode discovery, but it depends on the choice of $\beta$. The ablation simulation study in Fig.~\ref{fig:beta} shows that $\beta$ controls the balance between exploration and adherence to the prior in the sampling process~\eqref{eq:importancesampling}, under the same verification and policy update procedure. 
...
Ten rounds of expansion took approximately $1$ GPU hour on one NVIDIA H100 NVL GPU in the 3D cluttered task. 
Offline computation cost is dominated by uncertainty-guided self-generation~\eqref{eq:importancesampling} and the verification process~\eqref{eq:socp}, which account for \emph{$27\%$} and \emph{$24\%$}, respectively.
...
In Fig.~\ref{fig:coverage}(b), DCBF sample rejection adds only \(2.2\,\mathrm{ms}\) and increases the latency per replan from \(8.9\) to \(11.2\,\mathrm{ms}\) (\(25\%\)). MPPI-DCBF therefore remains practical as an online safety-critical planner while preserving the safety certificate.
...
Fig.~\ref{fig:tradeoff} quantitatively shows that $\gamma$ trades off safety and performance in both MPPI-DCBF and SFE. Across both OOD generalization tasks, lower $\gamma$ gives more conservative behavior with greater mean clearance, whereas higher $\gamma$ produces more aggressive behavior with shorter time-to-goal. This can be critical in real-world tasks where the robot must adjust caution to context, as $\gamma$ can serve as a knob that tunes aggressiveness or conservativeness at deployment.
...
With cost-based mode selection ($J=4,16$), SFE achieves even better performance and safety than $J=1$. This benefits from a larger candidate action pool at deployment at the cost of some computational overhead, as shown in Fig.~\ref{fig:coverage}(b).
...
\end{figure}
\section{Conclusion}
% \noindent\textbf{\emph{Takeaway 2.}} SFE discovers safe modes absent from the demonstrations, including the routes over and under obstacles in Fig.~\ref{fig:coverage}(a).
% \noindent\textbf{\emph{Takeaway 1.}} Offline expansion raises multi-step safety in every task and lowers the closed-loop collision rate. 
We presented a unified safe sampling-based planning paradigm. MPPI-DCBF certifies safety after MPPI's weighted averaging with modest computational overhead. Safe Flow Expansion uses offline computation to expand the generable set of CFM policies toward valid behaviors, enabling new safe modes in OOD environments. A shared safety parameter $\gamma$ tunes the conservativeness of both the online planner and generative policy.

Future work will study conditions for recursive feasibility of MPPI-DCBF under finite sampling, combine expanded CFM policies with lightweight run-time safety filters to provide hard safety guarantees, learn optimal $\gamma$ scheduling directly from data, and test robustness to simulator mismatch. 
