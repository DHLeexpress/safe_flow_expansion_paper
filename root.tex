...
First, MPPI-DCBF rejects rollouts that violate a common convex safety condition before MPPI's weighted averaging, preserving one-step safety. We pretrained a conditional flow matching (CFM) policy on MPPI-DCBF demonstrations.
...
In four OOD tasks, expansion improves validity over the pretrained policy. Hardware experiments demonstrate higher observed mode coverage than the pretrained and guided baselines. Single-action generation is approximately $\mathrm{12\times}$ and $\mathrm{50\times}$ faster than MPPI-DCBF and CFM-MPPI replanning, respectively.
\end{abstract}
...
Let $\mathcal I(\xk)\subset\mathbb N$ index the convex obstacles observed at $\xk$. For example, circular obstacles can be represented as 
\begin{equation}
\mathcal O_j:=\{\q\in\mathbb R^d\!:\!\|\q-\oj\|_2\!\le\! r_j\},
\label{eq:obstacle}
\end{equation}
For each $j\in\mathcal I(\xk)$, let $\p_j$ the closest point on obstacle to $\qk$ and $\n_j$ its outward unit normal. The robot is safe with respect to $\mathcal O_j$ when \(\n_j^\top(\qk-\p_j)>0\).
Define
\begin{equation}
...
We train CFM policies on MPPI-DCBF expert action sequences (hereafter, an \emph{action}), so that policy has a bias toward safe behaviors. 
%TODO safe?valid?
...
where the bounds $(1-\gamma)^i$ result from the finite-horizon DCBF condition~\eqref{eq:dtcbf}. Direct optimization with a non-convex $h$ does not generally guarantee recursive feasibility for this problem and often makes real-time implementation computationally intractable~\cite{rawlings2020model}.
...
% TODO: constraint/specification -> condition

In particular, the \emph{nominal} and \emph{verifier} polytopes in the following section define safety functions 
%whose superlevel sets specify the finite-horizon DCBF constraints 
...
\noindent\textbf{Extension: Multi-Body Points and Nonconvex Obstacles.} The affine-barrier construction in~\eqref{eq:aff_barrier} and  general convex obstacles, as is common in prior work~\cite{agrawal2017discrete, mizuta2025unified}.
...
Let $(\aj^\star,m_j^\star)$ a solution for~\eqref{eq:socp}, the verifier polytope for a queried action is defined as (See Fig.~\ref{fig:illustration} for illustration) 
\begin{equation}
...
\label{sec:sfe}
With the pretrained CFM policy, we now introduce Safe Flow Expansion (Alg.~\ref{alg:sfe}) to iteratively sample actions with uncertainty guidance and update the CFM policy to acquire safe valid actions absent from the demonstrations, including those excluded by MPPI-DCBF's demonstrations. 
...
Both valid and invalid queried actions update the uncertainty buffer until gathering $P$ safe trajectories. If no queried action is valid, the offline rollout restarts.
...
\noindent\textbf{Evaluation protocol.}
We evaluate the proposed framework in 4 OOD tasks using robots with double-integrator dynamics with $\Delta t\!=\!0.1\mathrm{s}$. We pretrain the policy on in-distribution MPPI-DCBF demonstrations, then conduct offline SFE in OOD environments and perform OOD evaluation.
...
\centering
    ...
    \vspace{-7.0mm}
    \caption{In a challenging dense and fast crowd environment, SFE successfully anticipates and executes valid actions, where $\gamma$ directly controls the valid actions, from conservative ($\gamma\!=\!0.1$) to aggressive behavior ($\gamma\!=\!1.0$).}
\label{fig:pedestrian}
\vspace{-8.0mm}
\end{figure}

\noindent \textbf{Implementation details:} 
The CFM velocity network is one MLP with 3 hidden layers, conditioned on $(\o,\gamma)$. We present local obstacle observations by discretizing the point-robot safety function $h=h^\mathrm{poly}$~\eqref{eq:poly_single} over a $2\mathrm{m}$ sensing range.
