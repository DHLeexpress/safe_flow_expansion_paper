...
We train CFM policies on MPPI-DCBF expert action sequences. Let $\hat{\mathbf u}^{\star}$ denote the optimized control sequence~\eqref{eq:mppi-update}, and set
...
\label{eq:cfm-loss}
\end{equation}
where $\square$ denotes \(t\sim\mathrm{Unif}[0,1],\epsilon\sim\mathcal{N}(0,I_{n_z}),(\z,\o,\gamma)\sim p_{\mathrm{data}}.\)
