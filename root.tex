...
We train CFM policies on MPPI-DCBF expert action sequences (hereafter, an \emph{action}), so that the policy has a bias toward safe behaviors. 
%TODO safe?valid?
...
At each iteration, Alg.~\ref{alg:sfe} samples toward actions with high uncertainty while adhering to the current policy~\footnote{The distribution in~\eqref{eq:importancesampling} is the exact Gibbs solution of \(\max_q \{\mathbb{E}_q[\sigma_n(\phi_s(\hat\u))]\!-\! \beta \operatorname{KL}(q \!\parallel\! p_1^{\theta_n})\},\) where $\operatorname{KL}$ denotes the Kullback--Leibler divergence.}
% ~\footnote{The exact Gibbs solution of
% \(
...
\noindent\textbf{Evaluation protocol.}
We evaluate the proposed framework in 4 OOD tasks using robots with double-integrator dynamics with $\Delta t\!=\!0.1\mathrm{s}$. We pretrain the policy on in-distribution MPPI-DCBF demonstrations, then conduct offline SFE in OOD, and perform OOD evaluation.
...
At deployment, SFE generates one action sequence and executes its first control before replanning. We additionally evaluate cost-based selection among $J>1$ generated sequences in~\Cref{sec:tradeoff}, where we generate $J$ action sequences and execute the first control of the sequence with the lowest rollout cost~\eqref{eq:mppicost}.
...
This empirical trend across rounds is qualitatively consistent with our analysis result in \Cref{thm:safe-action-availability}.
...
Future work will study conditions for recursive feasibility of MPPI-DCBF under finite sampling, combine expanded CFM policies with lightweight run-time safety filters to provide hard safety guarantees, and learn optimal $\gamma$ scheduling directly from data. 
