...
GPC connects general SBPCs with generative control by training FM policies on demonstrations from SBPCs~\cite{Kurtz_GPC}.
...
Active Flow Expansion~\cite{de2026active} is a recent active learning framework for generative models. Under proper conditions, tt enlarges the \textit{$\tau$-generable set} \(\Omega^\tau_\theta\!:=\!\{\x:p_1^\theta(\x)\ge\tau\}\) of the pretrained model $\theta$ to cover a larger portion of a valid region.
