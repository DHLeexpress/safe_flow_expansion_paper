...
and derive a lower bound on the probability that a generated batch contains a valid action at a fixed context and $\gamma$. We further show how the number of batches and $\gamma$ affect the safety-performance tradeoff.
...
In this paper, our safety specification is obstacle avoidance. We first model the robot as a point $\q=\piq(\x)$,
where $\piq:\mathbb R^{n_x}\to\mathbb R^d$ is a linear projection that maps the robot state to position. 
Let $\mathcal I(\xk)\subset\mathbb N$ index the obstacles observed at $\xk$.
...
Concretely, for a fixed observation and safety parameter \((\o,\gamma)\), let \(p_1^{\theta}(\cdot\!\mid\!\o,\!\gamma)\) denotes the model density at terminal flow time $t=1$. The CFM model $\theta$ learns to sample from the data distribution of \(p_\mathrm{data}(\z\!\mid\!\o,\!\gamma)\).
...
Active Flow Expansion~\cite{de2026active} enlarges the \textit{$\tau$-generable set} $\Omega^\tau_\theta\!:=\!\{\x:p_1^\theta(\x)\ge\tau\}$, the region an FM model samples with non-negligible probability under a finite budget~\cite{de2026active}.
...
We adopt this framework to expand our pretrained CFM policy's coverage of valid actions without additional expert demonstrations. Specifically, we use a finite-horizon DCBF verifier to determine sample validity. Section~\ref{sec:analysis} provides the theoretical analysis.
...
In particular, we define the \emph{nominal} and \emph{verifier} polytopes, whose superlevel sets specify the finite-horizon DCBF constraints for online MPPI and offline CFM policy expansion, respectively.
...
\end{equation}
Each pair of $\q^{(c)}$ and convex part $\mathcal O_{j}$ contributes one affine barrier function. The safety function~\eqref{eq:poly_single} can be extended as
\begin{equation}
h^{\mathrm{poly}}(\x;\xk)\!=\!\min_{c\le N_c}\min_{j\in\mathcal I(\xk)}
h^{\mathrm{aff}}_{j}\big(\pi_c(\x);\xk\big).
\label{eq:poly_multi}
\end{equation}
...
\centering
    ...
    \vspace{-7.0mm}
    \caption{\textbf{(a)} Illustrative example of MPPI-DCBF demonstration using nominal polytope \eqref{eq:poly_safeset} and the verifier polytope~\eqref{eq:verifierpolytope} to verify the finite-horizon DCBF condition \eqref{eq:socp} in the case of $H=3, d=2, \mathcal I(\xk) = \{1,2\}, \gamma=0.5$. The verifier admits collision-free queried actions that the nominal polytope excluded.
