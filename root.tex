...
We introduce MPPI-DCBF, which certifies safety against MPPI's averaging process by restricting rollouts to a common convex polytope, with modest computational overhead compared to vanilla MPPI.
...
\end{equation}
where $f,g$ are continuous and $k\in\mathbb Z$, $\x_k\in\mathcal X\subseteq\mathbb R^{n_x}$ is the state and $\u_k\in\mathcal U\subseteq\mathbb R^{n_u}$ is the control input. At each replanning step $k$, let $\x_{k+i}$ and $\u_{k+i}$ denote the predicted state and planned control within the current horizon, initialized at the current state $\x_k$.

\subsection{Model Predictive Path Integral control}
To control system~\eqref{eq:dynamical_system}, MPPI optimizes a mean control sequence in a convex admissible set $\mathcal U$ over horizon $H$ by sampling and cost-based reweighting~\cite{williams2016aggressive}.
It samples $M$ candidate sequences around the previous solution shifted one step, $\bar{\mathbf u}_{k+i}$, using $\mathbf u^{(m)}_{k+i}=\bar{\mathbf u}_{k+i}+\varepsilon^{(m)}_{k+i}$ with $\varepsilon^{(m)}_{k+i}\sim\mathcal N(0,\Sigma)$, for $m=1,\ldots,M$ and $i=0,\ldots,H-1$.
Each candidate is simulated from $\x_k$ using~\eqref{eq:dynamical_system} to evaluate its cost
\begin{equation}
\setlength{\jot}{0pt}
...
MPPI then updates the mean using weights $\omega_m=\exp(-(S^{(m)}-S_{\min})/\lambda)$, where $S_{\min}=\min_\ell S^{(\ell)}$ and $\lambda>0$ is the temperature:
\begin{equation}
\mathbf u^\star_{k+i}
=
...
% \end{align}
% \endgroup
% Only control $\uk^\star$ is applied before replanning. This parallelizable sampling process~\eqref{eq:mppi_rollout} is well-suited to solving non-convex optimal control problems on GPU hardware. However, a key challenge arises in safety-critical applications. Prior work incorporate DCBF-based penalties toward safety without certifying it~\cite{yin2023shield}, or executes among the lowest-cost rollouts under a CBF filter~\cite{rabiee2024guaranteed}.
