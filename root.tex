...
We define the safety specification that is derived from a supporting hyperplane to the single obstacle. Let \(q_{k+i \mid i}\in\mathbb R^d\) denote the robot position at state \(x_{k+i \mid i}\) at discrete time step $k$ and horizon $i = 0, ..., H-1$. For a convex obstacle, let $p$ be the point on its boundary closest to the current state $q_{k\mid k}$, with outward normal $n$. We define the barrier function as
\begin{equation}
h^{\mathrm{aff}}(q;\,q_{k\mid k}) :=
\frac{n^\top(q-p)}{n^\top(q_{k\mid k}-p)}.
\label{eq:aff_barrier}
\end{equation}
The implicit safe set for each horizon $i$ is defined as
\begin{equation}
\mathcal{C}_i^{\mathrm{aff}}=\{x \in \mathbb{R}^{n_x} \mid h^{\mathrm{aff}}(q; q_{k\mid k})\ge 0\},\label{eq:aff_safeset}
\end{equation}
is a half-space, which is convex. Iterating the condition \eqref{eq:dtcbf} from $i=0$ with the normalization $h^{\mathrm{aff}}(q_{k\mid k};q_{k\mid k})=1$ yields an exponentially decaying lower bound on the safety specification along the prediction horizon:
\begin{equation}
h^{\mathrm{aff}}(q_{k+i\mid k};q_{k\mid k}) \ge (1-\gamma)^i. \quad i = 0, ..., H.
...
\begin{equation}
\mathcal{P}_i=\{x \in \mathbb{R}^{n_x} \mid h^{\text{poly}}(\mathbf{q}_{k+i\mid k}; \mathbf{q}_{k\mid k})\ge (1-\gamma)^i\},\label{eq:poly_safeset}
\end{equation}
...
& h^{\text{poly}}(q_{k+i\mid k}; q_{k\mid k})\ge (1-\gamma)^i. \tag{1e} \label{eq:poly}
\end{align*}
\end{subequations}
...
\noindent \textbf{Proposition 1.} A stochastic policy $\mathbf{u}_{k:k+H-1\mid k}$ with length $H$ produced by MPPI-DCBF~\eqref{eq:mppi-dcbf}, which is the constrained stochastic optimization problem solution under the outward normal $n$ from \eqref{eq:aff_barrier}, is a valid multi-step samples:
\begin{equation*}
